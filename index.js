...
var state;
var startingBlock = ENV.STARTINGBLOCK || 42795206; //GENESIS BLOCK
const username = ENV.ACCOUNT || 'hashkings'; //account with all the SP
const key = steem.PrivateKey.from(ENV.KEY); //active key for account
const sh = ENV.sh || '';
const ago = ENV.ago || 42795206;
            ...
            if (
                ...
                if (state.stats.supply.land[want]) {
                    ...
                    if (want == 'market_seed') {
                       ...
                       if(json.seller && state.users[seller].seeds[0][seednames].owner === json.seller && state.users[from].seeds[0][seednames].forSale === true){
                         try{
                             for (var i = 0;i < 1; i++){
                                 if (json.seedPosted){
                                   if(state.users[seller].seeds[0][seednames] === json.seedPosted){
                                     state.users[seller].seeds[0][seednames].owner = json.from;
                                     purchasedSeed=state.users[seller].seeds[0][seednames].splice(i, 1)[0]
                                     break
                                   }
                                 } 
                             }
                         ...
                         if (purchasedSeed) {
                             if (!state.users[json.from]) {
                               ...
                               var pastValue = {
                                [json.block_num]: state.users[from].seeds[0][seednames].price
                               }
                               ...
                               state.users[from].seeds[0][seednames].datePosted = 0;
                                 ...
                                 state.users[json.to].seeds.push(purchasedSeed)
                             }
                             state.cs[`${json.block_num}:${from}`] = `${from} purchased a ${json.seedPosted} from ${json.seller}`
                         } else {
