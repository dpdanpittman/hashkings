...
var state;
var startingBlock = ENV.STARTINGBLOCK || 42647042; //GENESIS BLOCK
const username = ENV.ACCOUNT || 'hashkings'; //account with all the SP
const key = steem.PrivateKey.from(ENV.KEY); //active key for account
const sh = ENV.sh || '';
const ago = ENV.ago || 42647042;
...
processor.on('market_post_seed', function(json, from) {
    let seed = json.pollen,
        ...
        //for (var i = 0; i < seed.length; i++) {
            try {
            if (state.users[state.from].seeds[seed].owner === from && state.users[state.from].seeds[seed].forSale === false) {
                state.users[state.from].seeds[seed].forSale = true;
                seednames += `${seed[i]} `
            }
            } catch (e){
